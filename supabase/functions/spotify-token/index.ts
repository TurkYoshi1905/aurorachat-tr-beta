...
async function storeTokens(body: any): Promise<Response> {
  const { user_id, access_token, refresh_token, expires_in, display_name, email, spotify_id } = body;
  ...
  return json({ success: true, spotify_display_name: display_name || '', spotify_email: email || '' });
}
...
async function pollNowPlaying(user_id: string): Promise<Response> {
  ...
  if (!nowPlayingRes.ok) return json({ playing: false, reason: 'api_error' });
