...
**v1.0.4** — 28 Nisan 2026: performans, izinler ve "Kurallar" sayfası.
- **Discord-style mesaj render perf**: `ChatArea` üye lookup'ı her mesaj için `members.find()` (O(n)) yerine `useMemo` + Map (O(1)) ile yapılıyor; gradient hesaplamaları ve grup kararı memoize edildi.
- **Tüm rol izinleri tam aktif**: `send_messages`, `attach_files`, `pin_messages`, `manage_messages` izinleri artık `ChatArea` içinde gerçek zamanlı uygulanıyor. İzin yoksa composer disable olur ve dosya butonları gizlenir.
- **Rol rengi anlık yansıma**: `Index.tsx` içindeki `server_roles` realtime kanalı `fetchMembers()` tetikleyerek tüm üyelerin `roleColor`/`roleGradientEnd`'i güncellenir; `memberMap` rebuild olur ve chat'teki yazar rengi anında değişir.
- **Yeni `LoginBanModal`**: Login sayfasına özel, kapatılabilir ban modal'i (`AccountBanModal` non-dismissable, bu kullanıcı dostu varyant).
- **Kelime Filtresi - Muaf Roller arama çubuğu**: ServerSettings filter sekmesinde rolleri Türkçe-locale ile arama.
- **Yeni `/rules` sayfası** (`src/pages/Rules.tsx`): topluluk kuralları, ihlal türleri ve uygulanan ban süreleri (uyarı / 1 gün / 3 gün / 7 gün / 30 gün / kalıcı) modern bir tabloyla. PrivacyPolicy footer'ında büyük CTA üzerinden erişilir.
- **Sürüm bump 1.0.3 → 1.0.4**: package.json, src-tauri/tauri.conf.json, src-tauri/Cargo.toml, Landing footer, ReleaseNotesModal, Hakkında sekmesi, Tauri Windows EXE indirme bağlantıları ve `.github/workflows/tauri-build.yml` workflow_dispatch varsayılan değeri.

## Önceki Sürüm

**v1.0.3** — 27 Nisan 2026: kapsamlı kararlılık + özellik sürümü.
- Yeni kullanıcı kaydındaki **"Database error saving new user"** hatası giderildi: `handle_new_user` SECURITY DEFINER tetikleyicisi yeniden yazıldı; `profiles.username` UNIQUE çakışmalarını otomatik suffix ile çözüyor, geçersiz `gender`/`birth_date` değerleri sessizce normalize ediliyor ve istisna durumlarında auth akışını bloklamıyor (NOTICE ile loglanıyor).
- **Roller realtime**: `ServerSettings.tsx` artık `server_roles` + `server_member_roles` kanallarını birlikte dinleyip her iki sekmenin (Roller & Üyeler) içeriğini anında tazeliyor (`server_member_roles` kayıtları için sunucuya aitlik kontrolü ile filtreleniyor).
- **Bildirilerim realtime** (`Settings.ReportsSection`) zaten event'lere abone; v1.0.3 migration'ı `message_reports` için `REPLICA IDENTITY FULL` + publication garantisi ekleyerek payload tamlığını sağlıyor.
- **Beklenmedik çıkış (random sign-out) düzeltildi**: `AuthContext` artık yalnızca `SIGNED_OUT` event'inde profili temizliyor; `TOKEN_REFRESHED` boş geldiğinde mevcut session korunuyor; `INITIAL_SESSION`/`SIGNED_IN` dışında profil tekrar fetch edilmiyor.
- **Yeni globally-mounted modallar** (`App.tsx` içinde `<GlobalAuthModals />`):
  - `EmailVerificationModal` — login sonrası `email_confirmed_at` boşsa açılır; sağlayıcıya tek tık link, geri sayımlı tekrar gönderme, "Doğruladım" + çıkış butonları.
  - `AccountBanModal` — `accountBan` state'i (yeni `AuthContext` alanı) tetikler; ban sebebi + ban tarihi + itiraz yönergesi gösterir, kapatılamaz, sadece "Oturumu Kapat" ile çıkar. Ban watch realtime kanalı artık otomatik signOut+redirect yapmıyor.
  - `DeleteMessageConfirmModal` — `ChatArea`'da silme onayı; **Shift+Click** ile bypass (DM tarafında da aynı bypass mevcut).
- **Tauri masaüstü bildirimleri**: `Cargo.toml` `notification-all` feature, `tauri.conf.json` allowlist + `withGlobalTauri:true`, yeni `src/lib/tauriNotifications.ts` (web fallback dahil) ve `App.tsx` içinde `NotificationBootstrap` ile ilk açılışta izin istenir.
- **Gizlilik Politikası modernize edildi**: KVKK uyumu (veri sorumlusu, hukuki sebepler, KVKK 11 hakları), veri saklama süreleri, çocuk koruması, modern hero/section tasarımı.
- **Sürüm hizalama**: `package.json`, `src-tauri/tauri.conf.json`, `src-tauri/Cargo.toml`, `Landing` footer, `ReleaseNotesModal`, `Settings` (3 yer), `changelogData` v1.0.3.
- Yeni SQL migration: `supabase/migrations/20260427000000_v103_realtime_and_auth_fix.sql` (idempotent, `handle_new_user` rewrite + 5 tablo için `REPLICA IDENTITY FULL` + realtime publication).
