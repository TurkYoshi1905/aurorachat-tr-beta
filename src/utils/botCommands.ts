...
export async function executeBotCommand(
  ...
  switch (cmd) {
    ...
    default: {
      // Check custom bot commands from DB (server-specific bots)
      try {
        const { data: serverBots } = await supabase
          .from('server_bots')
          .select('bot_id')
        ...
        if (serverBots && serverBots.length > 0) {
          ...
          if (bots) {
            for (const bot of bots) {
              ...
              const commands: Array<{ trigger?: string; name: string; description: string; response: string }> =
                Array.isArray(bot.commands) ? bot.commands : [];
              const match = commands.find(c => {
                const normalizedCmd = c.name.toLowerCase().trim();
                return normalizedCmd === cmd;
              });
              if (match) {
                const callerMember = ctx.members.find(m => m.id === ctx.userId);
      ...
      // Unknown command — return null so the message is sent as a normal chat message
      return null;
    }
  }
}
