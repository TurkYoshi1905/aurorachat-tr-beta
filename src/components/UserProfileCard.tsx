...
import { MessageSquare, Moon, Smile, Bot, Zap, Gem, Star, CalendarDays, Server, ShieldCheck, UserPlus, UserCheck, UserX } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: prof } = await supabase
        .from('profiles')
        .select('display_name, username, avatar_url, updated_at, bio, banner_color, has_premium_badge, has_basic_badge, custom_status, premium_expires_at, basic_expires_at')
        .eq('id', userId)
        .maybeSingle();
      if (prof) {
        const now = new Date();
