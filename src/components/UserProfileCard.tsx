...
import { MessageSquare, Moon, Smile, Bot, Zap, Gem, Star, CalendarDays, Server, ShieldCheck, UserPlus, UserCheck, UserX, Smartphone, Tablet, ExternalLink, SkipBack, Play, Pause, SkipForward } from 'lucide-react';
...
const UserProfileCard = ({ userId, serverId, children, onSendMessage, status: externalStatus, platform: externalPlatform }: UserProfileCardProps) => {
  ...
  const [spotifyPlaying, setSpotifyPlaying] = useState<{
    is_playing: boolean;
  ...
  const spotifyRealtimeRef = useRef<ReturnType<typeof supabase.channel> | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const { data: prof } = await supabase
        .from('profiles')
        .select('display_name, username, avatar_url, updated_at, bio, banner_color, has_premium_badge, has_basic_badge, custom_status, premium_expires_at, basic_expires_at, platform')
        .eq('id', userId)
        .maybeSingle();
      if (prof) {
        const now = new Date();
  ...
  // Spotify now playing — fetch + real-time subscription
  useEffect(() => {
    if (!open || !userId || userId === AURORA_BOT_ID) return;

    const fetchSpotify = async () => {
      const { data } = await (supabase.from('spotify_now_playing') as any)
        .select('is_playing, track_name, artist_name, album_art_url, track_url, progress_ms, duration_ms')
        .eq('user_id', userId)
        .maybeSingle();
      setSpotifyPlaying(data?.is_playing ? data : null);
      setLocalProgressMs(data?.progress_ms ?? 0);
    };
    fetchSpotify();

    const ch = supabase
      .channel(`spotify-playing-${userId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'spotify_now_playing', filter: `user_id=eq.${userId}` }, (payload) => {
        const d = payload.new as any;
        setSpotifyPlaying(d?.is_playing ? d : null);
        setLocalProgressMs(d?.progress_ms ?? 0);
      })
      .subscribe();
    spotifyRealtimeRef.current = ch;
    return () => {
      if (spotifyRealtimeRef.current) { supabase.removeChannel(spotifyRealtimeRef.current); spotifyRealtimeRef.current = null; }
    };
  }, [open, userId]);
  ...
  useEffect(() => {
    ...
    if (spotifyPlaying?.is_playing) {
      progressIntervalRef.current = setInterval(() => {
        setLocalProgressMs(prev => {
          if (spotifyPlaying.duration_ms > 0 && prev >= spotifyPlaying.duration_ms) return prev;
          return prev + 1000;
        });
      }, 1000);
    }
