...
const DMDashboard = ({ onOpenDM, onStartCall, currentUserStatus = 'online', onStatusChange, presenceStatuses, onToggleNotifications, unreadNotifCount = 0, activeDMUser, onCloseDM, dmOnlineStatus, dmAutoStartVoice, dmInitiateCall, dmScrollToMessageId }: DMDashboardProps) => {
  const { user } = useAuth();
    ...
    <div className="flex-1 flex min-w-0 min-h-0 h-full overflow-hidden">
      ...
      <div className={`${isMobile ? (mobilePanelView === 'dms' ? 'flex' : 'hidden') : 'flex'} w-full sm:w-60 flex-shrink-0 bg-[#2b2d31] border-r border-[#1e1f22] flex-col overflow-hidden`} style={{ background: 'hsl(var(--sidebar))' }}>
        ...
        <nav className="px-2 mb-1 space-y-0.5">
          <button
            onClick={() => { setActiveDM(null); onCloseDM?.(); if (isMobile) setMobilePanelView('friends'); }}
              ...
              {filteredDMHistory.map((dm) => {
                ...
                return (
                  <div key={dm.userId} className="group relative">
                    <button
                      onClick={() => { setActiveDM(dm.userId); markDMRead(dm.userId); onOpenDM({ userId: dm.userId, displayName: dm.displayName, username: dm.username, avatarUrl: dm.avatarUrl }); }}
      ...
      <div className={`${isMobile ? (mobilePanelView === 'friends' ? 'flex' : 'hidden') : 'flex'} flex-1 flex-col min-w-0 overflow-hidden ${activeDMUser && !isMobile ? 'bg-background' : 'bg-[#313338]'}`}>
