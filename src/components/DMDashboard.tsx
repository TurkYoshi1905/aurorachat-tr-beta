...
import {
  ...
  PhoneCall, Video, ShieldOff, AlertTriangle, ChevronLeft, Bell, Plus
} from 'lucide-react';
import CreateGroupDMModal, { type FriendOption } from './CreateGroupDMModal';
...
interface DMDashboardProps {
  ...
  activeDMUser?: { userId: string; displayName: string; username: string; avatarUrl: string | null } | null;
  onCloseDM?: () => void;
  dmOnlineStatus?: string;
  dmAutoStartVoice?: boolean;
  dmInitiateCall?: boolean;
  dmScrollToMessageId?: string;
  externalPanel?: 'dms' | 'friends';
}
...
const DMDashboard = ({ onOpenDM, onStartCall, currentUserStatus = 'online', onStatusChange, presenceStatuses, onToggleNotifications, unreadNotifCount = 0, activeDMUser, onCloseDM, dmOnlineStatus, dmAutoStartVoice, dmInitiateCall, dmScrollToMessageId, externalPanel }: DMDashboardProps) => {
  const { user } = useAuth();
  ...
  // Sync externalPanel prop → mobilePanelView (for bottom nav tab control)
  useEffect(() => {
    if (externalPanel && isMobile) setMobilePanelView(externalPanel);
  }, [externalPanel, isMobile]);

  // Sync parent activeDMUser prop → internal activeDM state and clear unread dot
  useEffect(() => {
    if (activeDMUser?.userId) {
      setActiveDM(activeDMUser.userId);
      setUnreadDMs(prev => {
        const next = new Set(prev);
        next.delete(activeDMUser.userId);
        try { localStorage.setItem('aurorachat_unread_dms', JSON.stringify([...next])); } catch {}
        return next;
      });
    }
  }, [activeDMUser?.userId]);
  ...
  const [unreadDMs, setUnreadDMs] = useState<Set<string>>(() => {
    try {
      const stored = localStorage.getItem('aurorachat_unread_dms');
  ...
  const fetchFriends = useCallback(async () => {
    if (!user) return;
    const { data } = await supabase.from('friends').select('id, user_id, friend_id, status').or(`user_id.eq.${user.id},friend_id.eq.${user.id}`);
    if (!data) return;
    const otherUserIds = data.map((f: any) => f.user_id === user.id ? f.friend_id : f.user_id);
    let profilesMap = new Map<string, any>();
    if (otherUserIds.length > 0) {
      const { data: profiles } = await supabase.from('profiles').select('id, username, display_name, avatar_url, status').in('id', otherUserIds);
  ...
  const fetchDMHistory = useCallback(async () => {
    if (!user) return;
    try {
      const cached = await loadDMHistory(user.id);
      if (cached.length > 0) {
    ...
    try {
      ...
      const convList = (!convError && conversations) ? conversations : [];
      ...
      const hidden = await getHiddenDMs(user.id).catch(() => new Set<string>());
      const { data: profiles } = dmUsers.length > 0
        ? await supabase
            .from('profiles')
      ...
      try {
        const { data: myMemberships } = await supabase
          .from('group_dm_members' as any)
          .select('group_id')
          .eq('user_id', user.id)
          .eq('hidden', false);
        const groupIds = ((myMemberships as any[]) || []).map(m => m.group_id);
        if (groupIds.length > 0) {
          const { data: groups } = await supabase
            .from('group_dms' as any)
            .select('id, name, icon_url, owner_id, created_at, last_message_at')
            .in('id', groupIds);
          ...
          const { data: groupMsgs } = await supabase
            .from('group_dm_messages' as any)
            .select('group_id, content, inserted_at')
            .in('group_id', groupIds)
            .order('inserted_at', { ascending: false });
          ...
          // Auto-naming when no name set: collect missing profiles
          const missingProfileIds = new Set<string>();
          ((groups as any[]) || []).forEach(g => {
            if (!g.name) {
              (memberIdsByGroup.get(g.id) || []).forEach(uid => {
                if (uid !== user.id && !profileMap.has(uid)) missingProfileIds.add(uid);
              });
            }
          });
          ...
          const groupEntries: DMHistoryEntry[] = ((groups as any[]) || []).map(g => {
            const memberIds = memberIdsByGroup.get(g.id) || [];
            const otherIds = memberIds.filter(uid => uid !== user.id);
            const autoName = otherIds
              .slice(0, 3)
              .map(uid => (profileMap.get(uid) as any)?.display_name || (profileMap.get(uid) as any)?.username || 'Kullanıcı')
              .join(', ') || 'Grup';
        ...
        console.warn('group dm fetch failed', e);
      }

      // Sort merged list by lastAt desc
      entries.sort((a, b) =>
        new Date(b.lastAt || 0).getTime() - new Date(a.lastAt || 0).getTime()
      );

      setDmHistory(entries);
      await saveDMHistory(entries, user.id).catch(() => { });
    } catch { }
  }, [user, t]);
  ...
  const markDMUnread = (userId: string) => {
    setUnreadDMs(prev => {
      const next = new Set(prev);
      next.add(userId);
      try { localStorage.setItem('aurorachat_unread_dms', JSON.stringify([...next])); } catch {}
  ...
  // Real-time: direct_messages — when a new DM arrives, unhide the conversation and refresh DM list
  useEffect(() => {
    if (!user) return;
    const ch = supabase
      .channel('dm-dashboard-messages-realtime')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'direct_messages' }, async (payload) => {
        ...
        const { data: conv } = await supabase
          ...
          .maybeSingle();
        if (!conv) return;
        const partnerId = (conv as any).user1_id === user.id ? (conv as any).user2_id : (conv as any).user1_id;
        if (!partnerId) return;
        await restoreDMFromHistory(user.id, partnerId).catch(() => {});
        // Mark as unread if message is from the other user and that conversation is not currently open
        ...
        // Update the specific entry's lastMessage/lastAt and re-sort — avoids full refetch flicker
        setDmHistory(prev => {
          const existing = prev.find(d => d.userId === partnerId);
          let updated: DMHistoryEntry[];
          if (existing) {
            updated = prev.map(d =>
              d.userId === partnerId
  ...
  // This prevents stale DB "online" values from being shown after a user disconnects.
  const getStatus = useCallback((userId: string): string => {
    if (presenceStatuses?.has(userId)) return presenceStatuses.get(userId)!;
    if (presenceStatuses && presenceStatuses.size > 0) return 'offline';
  ...
  const handleDeleteDMHistory = async (userId: string, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await hideDMFromHistory(user!.id, userId);
  ...
  ).sort((a, b) => a.profile.displayName.localeCompare(b.profile.displayName, 'tr'));
  ...
  return (
    ...
    <div className="flex-1 flex min-w-0 min-h-0 h-full overflow-hidden">
      ...
      <div className={`${isMobile ? (mobilePanelView === 'dms' ? 'flex' : 'hidden') : 'flex'} w-full sm:w-60 flex-shrink-0 bg-[#2b2d31] border-r border-[#1e1f22] flex-col overflow-hidden`} style={{ background: 'hsl(var(--sidebar))' }}>
        ...
        <nav className="px-2 mb-1 space-y-0.5">
          <button
            onClick={() => { setActiveDM(null); setActiveGroupId(null); onCloseDM?.(); if (isMobile) setMobilePanelView('friends'); }}
          ...
          >
            <span className="relative w-4 h-4 flex-shrink-0">
              <Users className="w-4 h-4" />
              <Plus className="w-2.5 h-2.5 absolute -bottom-0.5 -right-1 bg-[#2b2d31] rounded-full p-[1px] text-[#3ba55d]" strokeWidth={3} />
            </span>
            <span className="text-sm font-medium">Grup Oluştur</span>
          </button>
        </nav>
              ...
              {filteredDMHistory.map((dm) => {
                ...
                return (
                  <div key={dm.userId} className="group relative">
                    <button
                      onClick={() => {
                        if (isGroup && dm.groupId) {
                          setActiveDM(dm.userId);
                          setActiveGroupId(dm.groupId);
                          setActiveGroupName(dm.displayName);
                          if (isMobile) setMobilePanelView('friends');
                          return;
                        }
                        setActiveDM(dm.userId);
                        setActiveGroupId(null);
                        markDMRead(dm.userId);
                        onOpenDM({ userId: dm.userId, displayName: dm.displayName, username: dm.username, avatarUrl: dm.avatarUrl });
                      }}
                    ...
                    >
                      <div className="relative flex-shrink-0">
                        <Avatar className="h-8 w-8">
                          ...
                          <AvatarFallback className={`text-white text-xs font-semibold ${isGroup ? 'bg-gradient-to-br from-[#5865f2] to-[#a855f7]' : 'bg-[#5865f2]'}`}>
                            {isGroup ? <Users className="w-3.5 h-3.5" /> : dm.displayName.charAt(0).toUpperCase()}
                          </AvatarFallback>
                        </Avatar>
                      ...
                      <div className="flex-1 min-w-0 text-left">
                        <div className="flex items-center justify-between">
                          <p className={`text-sm truncate leading-tight ${unreadDMs.has(dm.userId) ? 'font-bold text-white' : 'font-medium text-[#dbdee1]'}`}>{dm.displayName}</p>
                          <div className="flex items-center gap-1.5 flex-shrink-0 ml-1">
                            {dm.lastAt && (
                              <span className="text-[10px] text-[#6d6f78]">{formatRelativeTime(dm.lastAt)}</span>
                            )}
                            {unreadDMs.has(dm.userId) && !isActive && (
                              <span className="w-2 h-2 rounded-full bg-white shrink-0" />
                            )}
                          </div>
                        </div>
                        {dm.lastMessage && (
                          <p className={`text-[11px] truncate leading-tight ${unreadDMs.has(dm.userId) ? 'text-[#dbdee1] font-medium' : 'text-[#6d6f78]'}`}>{dm.lastMessage}</p>
                        )}
                      </div>
                    </button>
                    <button
      ...
      {/* Right: DM Chat or Friends Panel */}
      <div className={`${isMobile ? (mobilePanelView === 'friends' ? 'flex' : 'hidden') : 'flex'} flex-1 flex-col min-w-0 overflow-hidden ${(activeDMUser || activeGroupId) && !isMobile ? 'bg-background' : 'bg-[#313338]'}`}>
        {activeGroupId ? (
          <GroupDMChatArea
            key={activeGroupId}
            groupId={activeGroupId}
            groupName={activeGroupName}
            onBack={() => { setActiveGroupId(null); setActiveDM(null); if (isMobile) setMobilePanelView('dms'); }}
          />
        ) : activeDMUser && !isMobile ? (
          <DMChatArea
            key={activeDMUser.userId}
            dmUser={activeDMUser}
            onBack={onCloseDM || (() => {})}
            onlineStatus={dmOnlineStatus}
            autoStartVoice={dmAutoStartVoice}
            initiateCall={dmInitiateCall}
    ...
    <CreateGroupDMModal
      ...
      friends={friends.map<FriendOption>(f => ({
