...
const ChatArea = ({ channelName, channelId, messages, onSendMessage, onDeleteMessage, onEditMessage, onRetryMessage, onToggleMembers, showMembers, isOwner, isMobile, onBack, reactions, onToggleReaction, typingUsers, onTypingStart, onTypingStop, members = [], isLocked, onPinMessage, onUnpinMessage, serverId, threadCounts, onOpenThread, userPermissions, serverEmojis, onToggleSearch, onToggleNotifications, slowModeInterval, wordFilter, wordFilterExemptRoleIds, onBotMessage, hasMoreMessages, loadingMore, onLoadMore }: ChatAreaProps) => {
  ...
  // Memoize members lookup map for O(1) access during message render (Discord-style perf)
  ...
  // send_messages permission: owner, admin, or explicit grant. Default true if undefined (legacy).
  const canSendMessages = !!(
    isOwner ||
    userPermissions?.administrator ||
    userPermissions?.send_messages !== false
  );
  // attach_files permission: same fallback semantics
  const canAttachFiles = !!(
    isOwner ||
    userPermissions?.administrator ||
    userPermissions?.attach_files !== false
  );
  ...
  useEffect(() => {
    ...
    // If the first message changed and count grew, it's a "load more" prepend — don't scroll to bottom
    if (prevCount > 0 && currentFirstId !== prevFirstId && messages.length > prevCount) return;
  ...
  const handleMentionSelect = useCallback((name: string) => {
    ...
    if (mentionMatch) {
      ...
      setInput(`${beforeMention}${formatted} ${afterCursor}`);
    }
    setShowMentionPopup(false);
    setMentionQuery('');
    inputRef.current?.focus();
  ...
  const applyWordFilter = useCallback((text: string): string => {
    ...
    const currentMember = user?.id ? memberMap.get(user.id) : undefined;
    const isExempt = isOwner || !!userPermissions?.administrator || (!!currentMember?.roleId && !!wordFilterExemptRoleIds?.includes(currentMember.roleId));
    if (isExempt) return text;
    let filtered = text;
    ...
    return filtered;
  }, [wordFilter, wordFilterExemptRoleIds, isOwner, userPermissions?.administrator, memberMap, user?.id]);
      ...
      if (response) {
        ...
        if (botInsertError) {
          ...
          await supabase.from('messages').insert({
            ...
            user_id: user.id,
            author_name: response.botName || 'AuroraChat Bot',
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 min-h-0 h-full overflow-hidden">
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Popover open={showPinnedPanel} onOpenChange={setShowPinnedPanel}>
                ...
                <PopoverContent side="bottom" align="end" className="w-80 p-0" style={{ maxHeight: '420px', display: 'flex', flexDirection: 'column' }}>
      ...
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto overscroll-contain scrollbar-thin px-4 py-4" style={{ minHeight: 0 }}>
        ...
        {hasMoreMessages && (
          <div className="flex justify-center py-2">
            <button
              ...
              className="flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground bg-secondary/40 hover:bg-secondary/70 px-4 py-2 rounded-full transition-all disabled:opacity-50"
            >
              {loadingMore ? (
                <>
        ...
        {messages.map((msg, msgIndex) => {
          ...
          return (
            ...
            >
              {isGrouped ? (
                <div className="w-10 shrink-0 flex items-center justify-end pt-0.5">
                  <span className="text-[10px] text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity leading-none select-none">
                    {msg.timestamp.includes(' ') ? msg.timestamp.split(' ').slice(-1)[0] : msg.timestamp}
                  </span>
                </div>
              ) : (() => {
                const m = memberMap.get(msg.userId);
                return (
                  <UserProfileCard userId={msg.userId} serverId={serverId} status={m?.status} platform={m?.platform}>
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-baseline gap-2">
                  <UserProfileCard userId={msg.userId} serverId={serverId} status={member?.status} platform={member?.platform}>
                    {(() => {
                      ...
                      return (
                        <button
                          key={`author-${msg.userId}-${member?.roleColor ?? ''}-${member?.roleGradientEnd ?? ''}`}
                ...
                {msgReactions.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-1">
                    {msgReactions.map((r) => {
                      ...
                      const customMatch = /^:([a-zA-Z0-9_]+):$/.exec(r.emoji);
                      const customEmoji = customMatch ? serverEmojis?.find(e => e.name === customMatch[1]) : null;
                      return (
                        <button
                          key={r.emoji}
                          onClick={() => onToggleReaction?.(msg.id, r.emoji)}
                          className={`inline-flex items-center gap-1 text-xs px-1.5 py-0.5 rounded-md border transition-colors ${hasReacted ? 'bg-primary/20 border-primary/40 text-primary' : 'bg-secondary/50 border-border text-muted-foreground hover:bg-secondary'}`}
                          title={customEmoji ? `:${customEmoji.name}:` : undefined}
                        >
                          {customEmoji
                            ? <img src={customEmoji.image_url} alt={customEmoji.name} className="inline-block w-4 h-4 object-contain align-middle" />
                            : <span>{r.emoji}</span>}
              ...
              {editingId !== msg.id && !isMobileDevice && (
                <div className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 flex items-center gap-1 transition-all">
                      ...
                      <PopoverContent className="w-auto p-2" side="top" align="end">
                        <div className="space-y-2">
                          ...
                          {serverEmojis && serverEmojis.length > 0 && (
                            <>
                              <div className="text-[10px] uppercase tracking-wide text-muted-foreground px-1 pt-1">Sunucu Emojileri</div>
                              <div className="grid grid-cols-6 gap-1 max-h-40 overflow-y-auto">
                                {serverEmojis.map((se) => (
                                  <button
                                    key={se.id}
                                    onClick={() => onToggleReaction(msg.id, `:${se.name}:`)}
                                    title={`:${se.name}:`}
                                    className="w-8 h-8 flex items-center justify-center rounded hover:bg-secondary transition-colors"
                                  >
                                    <img src={se.image_url} alt={se.name} className="w-6 h-6 object-contain" />
                                  </button>
                                ))}
                              </div>
                            </>
                  ...
                  {(msg.userId === user?.id || isOwner || userPermissions?.manage_messages) && onDeleteMessage && (<button onClick={(e) => requestDeleteMessage(msg, e)} className="p-1 rounded hover:bg-destructive/20 text-muted-foreground hover:text-destructive transition-all" title={`${t('chat.deleteMessage')} (Shift: onaysız)`}><Trash2 className="w-3.5 h-3.5" /></button>)}
                  {msg.userId !== user?.id && (
                    <button
        ...
        <SheetContent side="bottom" className="rounded-t-2xl px-2 pb-[calc(env(safe-area-inset-bottom,0px)+16px)]">
          ...
          <div className="space-y-1">
            ...
            {onToggleReaction && longPressMsg && (
              <div className="py-2 border-b border-border mb-1 space-y-2">
                ...
                {serverEmojis && serverEmojis.length > 0 && (
                  <div className="flex justify-center gap-2 flex-wrap max-h-24 overflow-y-auto px-2">
                    {serverEmojis.slice(0, 16).map((se) => (
                      <button
                        key={se.id}
                        onClick={() => { onToggleReaction(longPressMsg.id, `:${se.name}:`); setLongPressMsg(null); }}
                        title={`:${se.name}:`}
            ...
            {longPressMsg && (longPressMsg.userId === user?.id || isOwner || userPermissions?.manage_messages) && onDeleteMessage && (
              <button onClick={() => { const m = longPressMsg; setLongPressMsg(null); requestDeleteMessage(m); }} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-destructive hover:bg-destructive/10 transition-colors">
                <Trash2 className="w-5 h-5" />
                {t('chat.deleteMessage')}
              </button>
            )}
            {longPressMsg && longPressMsg.userId !== user?.id && (
              <button
                onClick={() => { setReportMsg(longPressMsg); setLongPressMsg(null); }}
      ...
      {reportMsg && (
        <ReportMessageModal
          ...
          reportedUserId={reportMsg.userId}
          channelId={channelId}
          serverId={serverId}
        />
      )}
      ...
      <DeleteMessageConfirmModal
        ...
        onConfirm={() => {
          ...
          if (id && onDeleteMessage) onDeleteMessage(id);
        }}
      />
