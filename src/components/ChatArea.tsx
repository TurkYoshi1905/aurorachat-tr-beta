...
const renderBoldAndNewlines = (text: string): React.ReactNode => {
  const boldRegex = /\*\*(.+?)\*\*/g;
...
export const renderMessageContent = (content: string, currentUserId?: string, serverEmojis?: ServerEmoji[]) => {
  // Check if entire content is a single GIF URL (Giphy or Klipy)
  ...
  // Process custom server emojis :name: pattern — recursive for nested elements
  ...
  const handleSend = async () => {
    ...
    if (input.trim().startsWith('/') && serverId && channelId && user) {
      ...
      if (response) {
        ...
        // Insert bot response as a real message in DB
                  ...
                  <UserProfileCard userId={msg.userId} serverId={serverId} status={members.find(m => m.id === msg.userId)?.status}>
                    {(() => {
                      ...
                      const authorStyle: React.CSSProperties = hasGradient
                        ? { background: `linear-gradient(to right, ${member!.roleColor}, ${member!.roleGradientEnd}, ${member!.roleColor})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text' }
                        : member?.roleColor
                  ...
                  <>
                    {msg.content && (
                      msg.content.startsWith('📊 **ANKET**')
                        ...
                        : renderMessageContent(msg.content, user?.id, serverEmojis)
                    )}
