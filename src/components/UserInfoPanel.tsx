...
const UserInfoPanel = ({ currentUserStatus = 'offline', onStatusChange }: UserInfoPanelProps) => {
  ...
  const micMuted = voice.connected ? voice.micMuted : localMicMuted;
  const deafened = voice.connected ? voice.deafened : localDeafened;
  ...
  return (
    <TooltipProvider delayDuration={500}>
      <div
        className="h-[52px] flex items-center px-2 gap-1 shrink-0 select-none"
        ...
        <Popover open={statusOpen} onOpenChange={setStatusOpen}>
          <PopoverTrigger asChild>
            <button
              ...
              className="flex items-center gap-2 min-w-0 flex-1 h-full px-1 rounded-md hover:bg-white/5 active:bg-white/10 transition-colors cursor-pointer text-left overflow-hidden"
            >
              {/* Avatar */}
              ...
              {/* Name + username */}
              <div className="min-w-0 w-0 flex-1 flex flex-col justify-center leading-none gap-[3px]">
                <span
