...
const DMChatArea = ({ dmUser, onBack, onlineStatus, autoStartVoice, initiateCall, scrollToMessageId }: DMChatAreaProps) => {
  ...
  // Get or create conversation — also unhide it from DM history if it was hidden
  useEffect(() => {
    if (!user) return;
    const init = async () => {
      const convId = await getOrCreateConversation(user.id, dmUser.userId);
      setConversationId(convId);
      // Always unhide this conversation so it reappears in DM list when user goes back
      restoreDMFromHistory(user.id, dmUser.userId).catch(() => {});
    };
    init();
  }, [user, dmUser.userId]);
  ...
  useEffect(() => {
    ...
    const checkPrivacy = async () => {
      ...
      // Check if current user is blocked by the DM partner (use RPC to bypass RLS)
      const { data: blockedByOther } = await supabase.rpc('is_blocked_by', { p_blocker_id: dmUser.userId });
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      const { data } = await supabase.from('direct_messages')
        .select('id, sender_id, content, inserted_at, attachments, sender_deleted_user_id, conversation_id')
      ...
      if (data) {
        setMessages((data as any[]).map((m) => {
          const deletedSenderName = m.sender_deleted_user_id ? `Deleted User (${String(m.sender_deleted_user_id).slice(0, 8)})` : null;
          return {
            id: m.id, senderId: m.sender_id || m.sender_deleted_user_id || 'deleted-user', content: m.content, createdAt: m.inserted_at, updatedAt: null,
            senderName: deletedSenderName || (m.sender_id === user.id ? (profile?.display_name || 'Sen') : dmUser.displayName),
            senderAvatar: deletedSenderName ? null : (m.sender_id === user.id ? (profile?.avatar_url || null) : dmUser.avatarUrl),
  ...
  useEffect(() => {
    ...
    const handleInsert = (payload: any) => {
      ...
      setMessages((prev) => {
        ...
        const deletedSenderName = m.sender_deleted_user_id ? `Deleted User (${String(m.sender_deleted_user_id).slice(0, 8)})` : null;
        return [...prev, { id: m.id, senderId: m.sender_id || m.sender_deleted_user_id || 'deleted-user', content: m.content, createdAt: m.inserted_at, updatedAt: null, senderName: deletedSenderName || dmDisplayNameRef.current, senderAvatar: deletedSenderName ? null : dmAvatarUrlRef.current, attachments: m.attachments || undefined }];
      });
  ...
  useEffect(() => {
    ...
    const checkBlocking = async () => {
      ...
      const { data: blockedByOtherRpc } = await supabase.rpc('is_blocked_by', { p_blocker_id: dmUser.userId });
      if (blockedByOtherRpc) {
  ...
  const handleSend = useCallback(async (overrideContent?: string) => {
    ...
    const insertData: any = { id: tempId, sender_id: user.id, conversation_id: conversationId, content: content || '' };
    ...
    if (error) { setMessages((prev) => prev.map((m) => m.id === tempId ? { ...m, status: 'failed' as const } : m)); }
      ...
      setMessages((prev) => prev.map((m) => m.id === tempId ? { ...m, status: undefined, attachments: attachmentUrls } : m));
      // Broadcast real-time notification to recipient (works even when RLS blocks postgres_changes)
      const senderName = profile?.display_name || profile?.username || 'Biri';
      const pushBody = (content || '📎 Dosya').slice(0, 100);
      supabase.channel(`dm-notify-${dmUser.userId}`).httpSend({
        type: 'broadcast',
        event: 'new-dm',
        payload: {
          senderId: user.id,
      ...
      // Push notification is now sent server-side via DB trigger on notifications table
    }
  ...
  const handleRetrySend = useCallback(async (msg: DMMessage) => {
    ...
    const insertData: any = { id: msg.id, sender_id: user.id, conversation_id: conversationId, content: msg.content };
    ...
    else { setMessages((prev) => prev.map((m) => m.id === msg.id ? { ...m, status: undefined } : m)); }
  }, [user, profile, conversationId]);
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 min-h-0 h-full overflow-hidden bg-background relative">
        ...
        {messages.map((msg, idx) => {
            ...
            return (
              <div key={msg.id} className="flex items-center gap-2 px-3 py-2 mt-3 rounded-lg bg-destructive/10 border border-destructive/20">
                <div className="flex items-center gap-1.5 shrink-0">
                  <span className="text-[11px] font-bold text-destructive">AuroraChat</span>
                  <span className="text-[9px] font-bold uppercase text-destructive/80 bg-destructive/20 px-1.5 py-0.5 rounded tracking-wider">Sistem</span>
                </div>
                <span className="text-sm text-destructive/90">{msg.content}</span>
          ...
          return (
            <div
              ...
              } hover:bg-secondary/20 ${isMobile && selectedMsgId === msg.id ? 'bg-secondary/20' : ''}`}
            >
              <div className={`shrink-0 ${isMobile ? 'w-8' : 'w-9'}`}>
                {showAvatar ? (
                  <UserProfileCard userId={msg.senderId} status={msg.senderId === dmUser.userId ? dmUserStatus : undefined}>
                    <Avatar className={`${isMobile ? 'h-8 w-8' : 'h-9 w-9'} cursor-pointer`}>
              ...
              <div className="min-w-0 flex-1">
                {showAvatar && (
                  <div className="flex items-baseline gap-2">
                    <UserProfileCard userId={msg.senderId} status={msg.senderId === dmUser.userId ? dmUserStatus : undefined}>
                      <button className="font-semibold text-[13px] text-foreground hover:underline cursor-pointer">{msg.senderName}</button>
                    </UserProfileCard>
              ...
              {editingId !== msg.id && !msg.status && (
                <div className={`${isMobile ? (selectedMsgId === msg.id ? 'flex' : 'hidden') : 'hidden group-hover:flex'} items-center gap-0.5 shrink-0 self-start mt-1`}>
                  {own && (
                    <>
                      ...
                      <button
                        ...
                        title={`${t('dm.delete')} (Shift: onaysız)`}
                      >
        ...
        {hasBlockedOther ? (
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={dmBlocked}
              className="text-muted-foreground hover:text-foreground transition-colors shrink-0 p-1 flex items-center justify-center disabled:opacity-40"
            >
            ...
            <div className="flex-1 relative">
              <input
                ...
                className="w-full bg-input rounded-2xl py-3 pl-4 pr-10 text-[16px] outline-none text-foreground placeholder:text-muted-foreground ring-1 ring-border focus:ring-primary/40 transition-all min-h-[44px]"
              />
      ...
      {reportMsgDM && (
        <ReportMessageModal
          open={!!reportMsgDM}
          onClose={() => setReportMsgDM(null)}
          messageId={reportMsgDM.id}
          messageContent={reportMsgDM.content}
          reportedUserId={reportMsgDM.senderId}
      ...
      <Drawer open={galleryDrawerOpen} onOpenChange={setGalleryDrawerOpen} shouldScaleBackground={false}>
        <DrawerContent className="px-2 pb-[calc(env(safe-area-inset-bottom,0px)+16px)]">
          ...
          <div className="space-y-1 px-2 pb-3">
            <button
              onClick={() => { fileInputRef.current?.click(); setGalleryDrawerOpen(false); }}
