...
const ChangeGenderModal = ({ open, onOpenChange, currentValue, onSaved }: ChangeGenderModalProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg p-0 overflow-hidden bg-background border-border max-h-[92vh] flex flex-col gap-0 w-[calc(100%-1rem)] sm:w-full">
        {/* Hero */}
        <div className="relative px-4 sm:px-6 pt-4 sm:pt-6 pb-3 sm:pb-4 bg-gradient-to-br from-pink-500/15 via-purple-500/10 to-transparent border-b border-border shrink-0">
          <div className="flex items-start gap-3">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-pink-500/20 border border-pink-500/30 flex items-center justify-center shrink-0">
              <User2 className="w-5 h-5 sm:w-6 sm:h-6 text-pink-400" />
            </div>
            <div className="min-w-0 flex-1">
              <DialogHeader className="space-y-1 sm:space-y-1.5 text-left">
                <DialogTitle className="text-base sm:text-lg font-bold tracking-tight pr-6">Doğru Cinsiyetinizi Girin</DialogTitle>
                <DialogDescription className="text-[11px] sm:text-xs leading-relaxed">
                  Profilinde görünecek cinsiyeti seç. Bu bilgi gizlilik ayarına göre başkalarına gösterilebilir.
                </DialogDescription>
              </DialogHeader>
            </div>
          </div>
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-t border-border flex items-center justify-end gap-2 bg-secondary/20 shrink-0">
          <Button variant="ghost" size="sm" onClick={() => onOpenChange(false)} disabled={saving} className="sm:size-default">İptal</Button>
          <Button size="sm" onClick={handleSave} disabled={saving || !value || !canChange || isUnchanged} className="sm:size-default">
