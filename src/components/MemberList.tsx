...
import { ArrowLeft, Moon, Search, Crown, Shield, Star, Zap } from 'lucide-react';
  ...
  return (
    <UserProfileCard key={member.id} userId={member.id} serverId={serverId} status={member.status}>
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1 min-w-0">
            <p
              ...
              } ${!member.roleColor ? 'text-[#b5bac1] group-hover:text-[#dbdee1]' : ''}`}
              style={member.roleColor ? nameStyle : undefined}
            >
              {member.name}
            </p>
            {isCurrentUser && (
              <span className="text-[9px] bg-[#5865f2]/20 text-[#7289da] px-1 py-0.5 rounded font-bold uppercase shrink-0">
                Sen
              </span>
            )}
            {isPremium && (
        ...
        {member.role && (
          <div
            className="flex-shrink-0 flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-bold uppercase border opacity-80 group-hover:opacity-100 transition-opacity"
            style={
              member.roleColor
                ? { color: member.roleColor, borderColor: member.roleColor + '40', backgroundColor: member.roleColor + '15' }
                : { color: '#b5bac1', borderColor: '#3f414780', backgroundColor: '#3f414730' }
            }
          >
            {member.role}
          </div>
...
const MemberList = ({ members, isMobile, onBack, serverId, premiumUsers, currentUserId }: MemberListProps) => {
