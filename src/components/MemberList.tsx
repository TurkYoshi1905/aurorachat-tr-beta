...
const MemberRow = ({
  ...
  const effectiveStatus = member.isBot ? 'online' : member.status;

  return (
    <UserProfileCard key={member.id} userId={member.id} serverId={serverId} status={effectiveStatus} platform={member.platform} isBot={member.isBot} botName={member.isBot ? member.name : undefined} botAvatarUrl={member.isBot ? (member.avatarUrl ?? undefined) : undefined}>
      <div
      ...
      >
        <div className="relative flex-shrink-0">
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center text-sm overflow-hidden ring-2 transition-all ${
              effectiveStatus === 'offline' ? 'ring-transparent opacity-50' : 'ring-transparent group-hover:ring-white/10'
            }`}
          >
            {member.avatarUrl ? (
          ...
          {member.platform === 'mobile' && !member.isBot ? (
            ...
            <div
              className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-[#2b2d31] ${statusColor[effectiveStatus]}`}
            />
          )}
        </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-1 min-w-0">
            ...
            {member.isBot && (
              <span className="text-[8px] font-bold bg-[#5865f2]/20 text-[#5865f2] px-1 py-0.5 rounded uppercase leading-none shrink-0">BOT</span>
            )}
            {isPremium && !member.isBot && (
              <Zap className="w-3 h-3 text-yellow-400 shrink-0" />
            )}
          </div>
          {effectiveStatus !== 'offline' && (
            <p className="text-[10px] text-[#6d6f78] leading-tight truncate">
              {member.isBot ? 'Uygulama' : statusLabel[effectiveStatus]}
            </p>
          )}
        </div>
      </div>
...
// How stale last_seen must be before we consider the user offline.
// Heartbeat fires every 3 min (only while tab is visible).
// 8 min = 2× heartbeat + 2 min buffer.
const STALE_MS = 8 * 60 * 1000;

const MemberList = ({ members, isMobile, onBack, serverId, premiumUsers, currentUserId, ownerId }: MemberListProps) => {
  const { t } = useTranslation();
  const [search, setSearch] = useState('');
  // Re-evaluate staleness every 60 seconds without any server round-trip.
  ...
  // Apply last_seen staleness: if a non-bot member's last_seen is older than
  // STALE_MS, treat them as offline regardless of what the presence channel says.
  // This catches Android Chrome users whose WebSocket is kept alive in the
  // background but who have not been actively using the app.
  const effectiveMembers = useMemo(() => members.map(m => {
    if (m.isBot || !m.lastSeen || m.status === 'offline') return m;
    const stale = (now - new Date(m.lastSeen).getTime()) > STALE_MS;
    return stale ? { ...m, status: 'offline' as const } : m;
  }), [members, now]);

  const filtered = useMemo(() => {
    if (!search.trim()) return effectiveMembers;
    const q = search.toLowerCase();
    return effectiveMembers.filter(m => m.name.toLowerCase().includes(q) || m.username?.toLowerCase().includes(q));
  ...
  const onlineCount = effectiveMembers.filter(m => m.status !== 'offline').length;
  const totalCount = effectiveMembers.length;
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin px-1.5 py-2 space-y-0.5">
        ...
        {hasRoles ? (
          <>
            ...
            {(() => {
              const offlineAll = filtered.filter(m => m.status === 'offline');
