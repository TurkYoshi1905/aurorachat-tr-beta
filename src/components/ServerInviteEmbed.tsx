...
const ServerInviteEmbed = ({ code }: ServerInviteEmbedProps) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Removed broad profiles UPDATE listener (fired for every profile change globally, causing RPC storm).
      // Online count is now refreshed only when server membership changes, plus a periodic 2-minute refresh.
      ...
      // Periodic online count refresh every 2 minutes (replaces the noisy realtime profile listener)
      const periodicRefresh = setInterval(() => { if (!cancelled) refreshOnlineCount(); }, 2 * 60 * 1000);
      (memberCh as any)._periodicRefresh = periodicRefresh;
    };
    ...
    return () => {
      ...
      if (memberChannelRef.current) {
        const interval = (memberChannelRef.current as any)._periodicRefresh;
