...
const ServerInviteEmbed = ({ code }: ServerInviteEmbedProps) => {
  ...
  useEffect(() => {
    const loadInviteData = async () => {
      ...
      const { data: memberRows } = await supabase.from('server_members').select('id').eq('server_id', serverId);
      const memberCount = memberRows ? memberRows.length : 0;
  ...
  const handleJoin = async () => {
    ...
    if (!error) { setJoined(true); toast.success(t('invite.joinedSuccess', { name: server.name })); }
