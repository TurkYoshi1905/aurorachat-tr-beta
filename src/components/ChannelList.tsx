...
const ChannelList = ({ serverName, serverId, serverIcon, channels, categories = [], activeChannel, onChannelChange, currentUserStatus = 'offline', onStatusChange, isOwner, onChannelCreated, onServerDeleted, onServerUpdated, onLeaveServer, isMobile, voiceState, userPermissions }: ChannelListProps) => {
    ...
    return (
      <div key={channel.id} className="relative">
        ...
        <button
          ...
          onClick={() => isVoice ? voiceState?.joinVoice?.(channel.id, channel.name) : onChannelChange(channel.id)}
        ...
        >
          <Icon className={`w-[18px] h-[18px] shrink-0 ${isActive ? 'opacity-90' : 'opacity-60 group-hover:opacity-80'} ${isVoiceActive ? 'text-status-online' : ''}`} />
          <span className="truncate font-medium text-[14px]">{channel.name}</span>
