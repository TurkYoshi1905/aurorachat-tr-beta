...
export const useVoice = (): VoiceState => {
  ...
  const toggleMic = useCallback(() => {
    ...
    roomRef.current.localParticipant.setMicrophoneEnabled(!newMuted, AURORA_AUDIO_CAPTURE_DEFAULTS as any);
  ...
  const toggleDeafen = useCallback(() => {
    ...
    if (newDeafened && !micMuted) {
      ...
      roomRef.current.localParticipant.setMicrophoneEnabled(true, AURORA_AUDIO_CAPTURE_DEFAULTS as any);
      setMicMuted(false);
    }
