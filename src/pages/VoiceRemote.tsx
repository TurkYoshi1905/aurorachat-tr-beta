...
const VoiceRemote = () => {
  ...
  const phoneCameraUrl = channelId
    ? `/voice-join?channelId=${encodeURIComponent(channelId)}&channelName=${encodeURIComponent(channelName)}`
    : '';
  ...
  const handleToggleCamera = () => sendCommand('toggle_camera');
  const handleLeave = () => {
  ...
  return (
    <div className="h-screen w-screen flex flex-col bg-[#0f0f0f] overflow-hidden select-none">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-6 px-6">
        ...
        {phoneCameraUrl && (
          <button
            onClick={() => navigate(phoneCameraUrl)}
            className="px-4 py-2 rounded-xl text-xs font-semibold"
            style={{ background: 'rgba(0,255,136,0.12)', color: '#00ff88', boxShadow: '0 0 0 1px rgba(0,255,136,0.28)' }}
          >
            Telefonun kamerasıyla kanala katıl
          </button>
        )}
      </div>
