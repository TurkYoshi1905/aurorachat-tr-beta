...
const InvitePage = () => {
  ...
  const handleJoin = async () => {
    ...
    try {
      const { data: srv } = await supabase.from('servers').select('welcome_enabled, welcome_message, welcome_channel_id, name').eq('id', server.id).single();
      if (srv && (srv as any).welcome_enabled && (srv as any).welcome_message && (srv as any).welcome_channel_id) {
        ...
        const content = ((srv as any).welcome_message as string)
          ...
          .replace(/\{server\}/g, (srv as any).name || server.name);
