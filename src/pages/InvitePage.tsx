...
const InvitePage = () => {
  ...
  useEffect(() => {
    const fetchInvite = async () => {
      ...
      setServer({ id: srv.id, name: srv.name, icon_url: srv.icon_url, memberCount: count || 0 });
      setLoading(false);
    };
    fetchInvite();
  }, [code, user]);
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="w-full max-w-sm rounded-2xl border border-border bg-card overflow-hidden shadow-xl">
          ...
          <div className="h-24 bg-gradient-to-br from-primary/20 via-primary/10 to-transparent relative overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-primary/30 to-transparent" />
          </div>

          <div className="px-8 pb-8 -mt-8 space-y-6">
            {/* Logo */}
            <div className="flex flex-col items-center gap-2">
              ...
              <h1 className="text-xl font-bold text-foreground">AuroraChat</h1>
            </div>

            <div className="text-center">
              <h2 className="text-base font-semibold text-foreground mb-1">Sunucuya Katıl</h2>
              <p className="text-sm text-muted-foreground">
                Bu davete erişmek için hesabınıza giriş yapın.
              </p>
            </div>
      ...
      <div className="w-full max-w-sm rounded-2xl border border-border bg-card overflow-hidden shadow-xl">
        ...
        <div className="px-8 pb-8 -mt-12 space-y-4">
          ...
          <div className="text-center space-y-1">
            <h2 className="text-xl font-bold text-foreground" data-testid="text-server-name">{server?.name}</h2>
            <div className="flex items-center justify-center gap-1.5 text-sm text-muted-foreground">
              <span className="w-2 h-2 rounded-full bg-emerald-500 inline-block" />
              <Users className="h-3.5 w-3.5" />
              <span data-testid="text-member-count">{server?.memberCount.toLocaleString('tr-TR')} üye</span>
            </div>
          </div>
