...
const Index = () => {
  ...
  // Global DM notification listener — creates notifications for ALL incoming DMs
  // even when DMChatArea is not open for that conversation
  useEffect(() => {
    if (!user) return;
      ...
      .on(
        ...
        async (payload) => {
          ...
          if ('Notification' in window && Notification.permission === 'granted') {
            new Notification(notifTitle, { body: notifBody, icon: '/favicon.ico', tag: `dm-${msg.id}` });
          }
        }
      )
      .subscribe();
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'server_members', filter: `server_id=eq.${activeServer}` }, () => {
        fetchMembers();
      })
      .subscribe();
    return () => { supabase.removeChannel(ch); };
  }, [activeServer, fetchMembers]);
        ...
        if (m.channel_id === channelRef.current) {
          if (m.user_id === userRef.current) {
            ...
            setMessages((prev) => {
              ...
              // Only apply hasTempMsg de-dup for real user messages, never for bot messages
              if (!isBot) {
              ...
              return [...prev, {
                ...
                avatarUrl: isBot ? '/aurora-bot-avatar.jpg' : null, userId: isBot ? 'aurora-bot' : m.user_id,
                isBot, content: m.content, timestamp: formatTimestamp(m.inserted_at),
          ...
          // Insert mention notification if current user is mentioned
          if (!isBot && userRef.current && profile) {
            ...
            if (isMentioned) {
              ...
              if ('Notification' in window && Notification.permission === 'granted') {
                new Notification(notifTitle, { body: notifBody, icon: '/favicon.ico', tag: `mention-${m.id}` });
              }
              // Push notification (for offline/background case)
              if (userRef.current) {
  ...
  useEffect(() => {
    const globalRealtime = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'server_member_roles' }, () => { fetchMembers(); })
  ...
  useEffect(() => {
    ...
    presenceChannel
      .on('presence', { event: 'sync' }, () => {
        const state = presenceChannel.presenceState<{ status: string }>();
        ...
        setMembers((prev) => prev.map((m) => ({
          ...m, status: (presenceMap.get(m.id) as DbMember['status']) || 'offline',
        })));
      })
      .subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          await presenceChannel.track({ status: myStatus });
  ...
  useEffect(() => {
    ...
    const key = 'aurorachat_update_toast_048';
    if (sessionStorage.getItem(key)) return;
    sessionStorage.setItem(key, 'true');
    setTimeout(() => {
      toast.success('🎉 Yeni Güncelleme! AuroraChat 0.4.8 yayınlandı! Sesli sohbet iyileştirmelerini deneyin.');
    }, 1500);
  }, [splashDone, user]);
          ...
          showVoiceChat ? (
            ...
            <div className="flex flex-1 min-w-0 overflow-hidden flex-col">
              ...
              <div className="flex flex-1 min-w-0 overflow-hidden">
                ...
                {showMembers && !showSearchPanel && <MemberList members={members} serverId={activeServer} currentUserId={user?.id} />}
              </div>
            </div>
          ) : (
