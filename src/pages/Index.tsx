...
import { useServerOnlineBroadcast } from '@/hooks/useServerOnlineBroadcast';
import { appCache, debounce } from '@/lib/queryCache';
...
const Index = () => {
  ...
  useServerOnlineBroadcast(user?.id, servers.map(s => s.id), myStatus);
  ...
  const messagesRealtimeChannelRef = useRef<ReturnType<typeof supabase.channel> | null>(null);
  const presenceStatusRef = useRef<Map<string, string>>(new Map());
  const presencePlatformGlobalRef = useRef<Map<string, DbMember['platform']>>(new Map());
  const myStatusRef = useRef<DbMember['status']>('online');
  // Tracks users who were explicitly marked offline via DB Realtime (beforeunload/signout).
  // Prevents presence sync from overriding this until the user reconnects.
  const dbOfflineUsersRef = useRef<Set<string>>(new Set());
  ...
  // When the user backgrounds the app or closes it, last_seen stops updating.
  ...
  const fetchServers = useCallback(async () => {
    const cacheKey = `servers:${user?.id}`;
    // If app is already navigated, serve from cache to avoid redundant Supabase calls.
    // Cache is invalidated when realtime events fire (channel/server/member changes).
    if (activeServerRef.current) {
      const cached = appCache.get<DbServer[]>(cacheKey);
    ...
    const channelsQuery = async () => {
      const res = await (supabase as any)
        .from('channels')
        .select('id, name, type, position, sort_order, server_id, category_id, is_locked, slow_mode_interval')
      ...
      return (supabase as any)
        .from('channels')
        .select('id, name, type, position, server_id, category_id, is_locked, slow_mode_interval')
        .in('server_id', serverIds)
        .order('position', { ascending: true });
    };

    const [{ data: serversData }, { data: channelsData }, { data: categoriesData }] = await Promise.all([
      supabase.from('servers').select('id, name, icon, owner_id, word_filter, word_filter_exempt_role_ids, created_at').in('id', serverIds).order('created_at', { ascending: true }),
      channelsQuery(),
      supabase.from('channel_categories').select('id, name, position, server_id').in('server_id', serverIds).order('position', { ascending: true }),
    ]);
    ...
    if (serversData && channelsData) {
      const mapped: DbServer[] = serversData.map((s) => ({
        ...
        word_filter_exempt_role_ids: (s as any).word_filter_exempt_role_ids || [],
        channels: channelsData
          ...
          .map((c) => ({
            ...
            position: (c as any).sort_order ?? c.position,
      ...
      // Cache for 60 seconds — invalidated by realtime events below
      appCache.set(`servers:${user?.id}`, mapped, 60_000);
      setServers(mapped);
          ...
          try {
            ...
            if (nav.serverId === 'home') {
              ...
              if (srv) {
                ...
                const fallbackCh = srv.channels.find((c: any) => c.type === 'text') || srv.channels[0];
                setActiveChannel(ch ? nav.channelId : (fallbackCh?.id || ''));
              } else {
                setActiveServer(mapped[0].id);
                const fc0 = mapped[0].channels.find(c => c.type === 'text') || mapped[0].channels[0];
                setActiveChannel(fc0?.id || '');
              }
            } else {
              setActiveServer(mapped[0].id);
              const fc1 = mapped[0].channels.find(c => c.type === 'text') || mapped[0].channels[0];
              setActiveChannel(fc1?.id || '');
            }
          } catch {
            setActiveServer(mapped[0].id);
            const fc2 = mapped[0].channels.find(c => c.type === 'text') || mapped[0].channels[0];
            setActiveChannel(fc2?.id || '');
          }
        } else {
          setActiveServer(mapped[0].id);
          const firstChannel = mapped[0].channels.find(c => c.type === 'text') || mapped[0].channels[0];
  ...
  useEffect(() => {
    if (!user) return;
    // Güvenlik ağı: 12 saniye sonra tüm adımları zorla tamamla
    ...
    const initApp = async () => {
      ...
      // fetchServers için 10 saniye zaman aşımı
      await Promise.race([
        fetchServers(),
    ...
    initApp();
    return () => clearTimeout(forceFinish);
  }, [user?.id]);

  // Fetch members — single RPC call (4 queries → 1), cached per server for 60s
  const fetchMembers = useCallback(async () => {
    if (!activeServer || activeServer === 'home') return;
    const cacheKey = `members:${activeServer}`;

    // Return presence-refreshed cache when available (no Supabase roundtrip)
    const cached = appCache.get<DbMember[]>(cacheKey);
    if (cached) {
      setMembers(cached.map(m => {
        if (m.isBot) return { ...m, status: 'online' as const };
        return {
          ...m,
          status: (presenceStatusRef.current.get(m.id) as DbMember['status']) || (m.id === userRef.current ? myStatusRef.current : m.status),
          platform: presencePlatformGlobalRef.current.get(m.id) ?? m.platform,
        };
      }));
      return;
    }

    // Single RPC call replaces 3 parallel Supabase queries
    const { data: rows, error } = await (supabase as any).rpc('get_server_members_full', { p_server_id: activeServer });
    if (error || !rows) {
      // Fallback to legacy 3-query approach on RPC error
      const { data: memberRows } = await supabase.from('server_members').select('id, user_id').eq('server_id', activeServer);
      if (!memberRows) return;
      const userIds = memberRows.map((m: any) => m.user_id);
      if (userIds.length === 0) { setMembers([]); return; }
      const memberIds = memberRows.map((m: any) => m.id);
      const [profilesRes, memberRolesRes, serverRolesRes, serverBotsRes] = await Promise.all([
        supabase.from('profiles').select('id, username, display_name, avatar_url, status, last_seen, bio, banner_color, is_premium, has_premium_badge, has_basic_badge').in('id', userIds),
        ...
        (supabase as any).from('server_bots').select('id, bot_id, bots(id, name, username, avatar_url, description)').eq('server_id', activeServer),
      ]);
      ...
      const humanMembers: DbMember[] = memberRows.map((m: any) => {
        const p = profiles.find((pr: any) => pr.id === m.user_id);
        const userRoleIds = memberRoles.filter((mr: any) => mr.member_id === m.id).map((mr: any) => mr.role_id);
        const userRoles = serverRoles.filter((r: any) => userRoleIds.includes(r.id));
        const topRole = userRoles.length > 0 ? userRoles[0] : null;
        const displayName = (p as any)?.display_name || (p as any)?.username || 'Kullanıcı';
        return {
          ...
          roleId: topRole?.id, roleGradientEnd: topRole?.permissions?.['gradient_end_color'] || undefined,
        };
      });
      const botMembers: DbMember[] = ((serverBotsRes.data || []) as any[]).map((sb: any) => {
        const b = sb.bots;
        if (!b) return null;
        const displayName = b.name || b.username || 'Bot';
        return {
          id: b.id, name: displayName, username: b.username || '',
          avatar: displayName.charAt(0).toUpperCase(), avatarUrl: b.avatar_url || null,
          status: 'online' as const, isBot: true,
        };
      }).filter(Boolean) as DbMember[];
      const fallback: DbMember[] = [...humanMembers, ...botMembers];
      appCache.set(cacheKey, fallback, 60_000);
      setMembers(fallback);
      return;
    }

    // Map RPC result (already sorted by position DESC from DB)
    const mapped: DbMember[] = (rows as any[]).map((m: any) => {
      const roles: any[] = m.roles || [];
      const topRole = roles.length > 0 ? roles[0] : null;
      const displayName = m.display_name || m.username || 'Kullanıcı';
  ...
  const fetchPerms = useCallback(async () => {
    ...
    const cacheKey = `perms:${activeServer}:${user.id}`;
    const cached = appCache.get<Record<string, boolean>>(cacheKey);
    if (cached) { setUserPermissions(cached); return; }
    ...
    if (roles) {
      const perms = getHighestPermissions(roles.map((r: any) => ({ permissions: r.permissions || {} })));
  ...
  useEffect(() => {
    ...
    const ch = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'server_member_roles' }, () => {
        ...
        debouncedServersRefresh();
        debouncedPermsRefresh();
      })
      ...
      // server_bots: bot eklenince/çıkarılınca üye listesini yenile
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'server_bots', filter: `server_id=eq.${activeServer}` }, () => {
        debouncedMembersRefresh();
      })
      .on('postgres_changes', { event: 'DELETE', schema: 'public', table: 'server_bots' }, () => {
        debouncedMembersRefresh();
      })
      .subscribe();
  ...
  useEffect(() => {
    ...
    const cacheKey = `emojis:${activeServer}`;
    const cached = appCache.get<{ id: string; name: string; image_url: string }[]>(cacheKey);
    if (cached) { setServerEmojis(cached); return; }
    supabase.from('server_emojis').select('id, name, image_url')
      .eq('server_id', activeServer)
      .then(({ data }) => {
        const emojis = data || [];
        appCache.set(cacheKey, emojis, 300_000);
  ...
  useEffect(() => {
    ...
    const mapRawMessages = async (rawData: any[]): Promise<DbMessage[]> => {
      const userIds = [...new Set(rawData.map((m) => m.user_id).filter(Boolean))];
      const [profilesResult] = await Promise.all([
        userIds.length > 0
          ? supabase.from('profiles').select('id, avatar_url, display_name, username').in('id', userIds)
          : Promise.resolve({ data: [] }),
      ]);
      const avatarMap = new Map(profilesResult.data?.map((p: any) => [p.id, p.avatar_url]) || []);
      ...
      return rawData.map((m) => {
        ...
        const liveName = isBot ? botDisplayName! : (nameMap.get(m.user_id) || (m as any).author_name || 'Kullanıcı');
        return {
          id: m.id,
    ...
    const fetchMessages = async () => {
      const { data } = await supabase
        ...
        .limit(PAGE_SIZE)
        .then(r => ({ ...r, data: r.data ? [...r.data].reverse() : r.data }));

      if (data) {
        setHasMoreMessages(data.length === PAGE_SIZE);
        const mapped = await mapRawMessages(data);
        setMessages(mapped);

        const messageIds = data.map((m) => m.id).filter((id: string) => !String(id).startsWith('bot-'));
  ...
  const loadMoreMessages = useCallback(async () => {
    if (loadingMore || !activeChannel || activeChannel === 'home') return;
    setLoadingMore(true);
    const PAGE_SIZE = 100;
    ...
    const mapRawMessages = async (rawData: any[]): Promise<DbMessage[]> => {
      ...
      const profilesResult = userIds.length > 0
        ? await supabase.from('profiles').select('id, avatar_url, display_name, username').in('id', userIds)
        : { data: [] };
      const avatarMap = new Map((profilesResult.data || []).map((p: any) => [p.id, p.avatar_url]));
      ...
      return rawData.map((m: any) => {
        ...
        const liveName = isBot ? botDisplayName! : (nameMap.get(m.user_id) || m.author_name || 'Kullanıcı');
        return {
          id: m.id,
          author: deletedAuthor || liveName,
          avatar: (deletedAuthor ? 'D' : liveName).charAt(0).toUpperCase(),
    ...
    try {
      setMessages(prev => {
        const oldest = prev[0];
        if (!oldest || !oldest.insertedAt) { setLoadingMore(false); return prev; }
        const oldestTs = oldest.insertedAt;
        ...
        supabase
          ...
          .limit(PAGE_SIZE)
          .then(async r => {
            const data = r.data ? [...r.data].reverse() : [];
            if (data.length > 0) {
              setHasMoreMessages(data.length === PAGE_SIZE);
              const mapped = await mapRawMessages(data);
  ...
  }, [activeChannel, loadingMore, formatTimestamp]);
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, async (payload) => {
        ...
        if (m.channel_id === channelRef.current) {
          ...
          if (m.user_id === userRef.current) {
            setMessages((prev) => {
              ...
              return [...prev, {
                ...
                avatarUrl: profile?.avatar_url || null, userId: m.user_id,
          ...
          setMessages((prev) => {
            if (prev.some((msg) => msg.id === m.id)) return prev;
            // Prefer the live display_name from profiles so renamed users update everywhere.
            const liveAuthor = rtDeletedAuthor
              || (isBot ? (m.author_name || 'AuroraChat Bot') : ((prof as any)?.display_name || (prof as any)?.username || m.author_name || 'Kullanıcı'));
            return [...prev, {
      ...
      .on('broadcast', { event: 'message_deleted' }, ({ payload }: any) => {
        ...
        if (payload.channelId === channelRef.current) {
          setMessages((prev) => prev.filter((m) => m.id !== payload.messageId));
          setReactions((prev) => {
            if (!prev[payload.messageId]) return prev;
            const next = { ...prev };
            delete next[payload.messageId];
            return next;
          });
          if (payload.deletedBy && payload.deletedBy !== userRef.current) {
            toast.info('Bir mesaj silindi');
          }
        }
      })
      ...
      .subscribe();
    messagesRealtimeChannelRef.current = channel;

    return () => {
      if (messagesRealtimeChannelRef.current === channel) messagesRealtimeChannelRef.current = null;
  ...
  useEffect(() => {
    ...
    const globalRealtime = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'channel_categories' }, () => { invalidateAndFetchServers(); })
      .on('postgres_changes', { event: '*', schema: 'public', table: 'servers' }, () => { invalidateAndFetchServers(); })
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'server_members' }, () => { invalidateAndFetchMembers(); invalidateAndFetchServers(); })
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles' }, (payload) => {
        ...
        const presenceStatus = presenceStatusRef.current.get(updated.id) as DbMember['status'] | undefined;

        // DB 'offline' is an explicit signal (beforeunload / signOut).
        // Record it so the presence-sync handler cannot override it until
        // the user reconnects and sends a non-offline DB update.
        if (dbStatus === 'offline') {
          dbOfflineUsersRef.current.add(updated.id);
        } else if (dbStatus) {
          dbOfflineUsersRef.current.delete(updated.id);
        }
        ...
        setMembers((prev) => prev.map((m) => {
          ...
          return {
            ...
            platform: updated.platform || m.platform,
            lastSeen: updated.last_seen || m.lastSeen,
          };
        }));
        setMessages((prev) => prev.map((msg) => msg.userId === updated.id ? { ...msg, author: newName, avatarUrl: updated.avatar_url || msg.avatarUrl } : msg));
      })
      .subscribe();
  ...
  useEffect(() => {
    ...
    presencePlatformGlobalRef.current.set(user.id, myPlatform);
    presenceChannel
      .on('presence', { event: 'sync' }, () => {
        ...
        for (const [userId, presences] of Object.entries(state)) {
          ...
          if (latest?.platform) {
            presencePlatformGlobalRef.current.set(userId, latest.platform as DbMember['platform']);
          }
          // User appeared in presence → they reconnected; lift the DB-offline lock
          dbOfflineUsersRef.current.delete(userId);
        }
        presenceStatusRef.current = presenceMap;
        setPresenceStatuses(new Map(presenceMap));
        setMembers((prev) => prev.map((m) => {
          // Bots are never in the presence channel — always keep them online
          if (m.isBot) return { ...m, status: 'online' as const };
          // DB explicitly said this user is offline (beforeunload/signOut fired).
          // Keep them offline even though the presence channel hasn't timed them out yet.
          if (dbOfflineUsersRef.current.has(m.id)) {
            return { ...m, platform: presencePlatformGlobalRef.current.get(m.id) || m.platform };
          }
          return {
            ...m,
            // Not in channel = offline; in channel = their tracked status
      ...
      .subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          ...
          // Write status + platform + last_seen to DB on every presence connect.
          // This corrects any stale "offline" that prior page unloads may have written.
  ...
  useEffect(() => {
    ...
    window.addEventListener('aurorachat:navigate-to-server', handler);
    return () => window.removeEventListener('aurorachat:navigate-to-server', handler);
  }, [user, servers, isMobile]);
  ...
  const rawChannel = server?.channels.find((c) => c.id === activeChannel) || server?.channels[0];
  const channel = rawChannel?.type === 'voice' ? (server?.channels.find(c => c.type === 'text') || rawChannel) : rawChannel;
  ...
  const handleServerChange = useCallback((id: string) => {
    ...
    if (s) { setActiveServer(id); const firstChannel = s.channels.find(c => c.type === 'text') || s.channels[0]; if (firstChannel) setActiveChannel(firstChannel.id); if (isMobile) setMobileTab('channels'); }
  }, [servers, isMobile]);
    ...
    if (newServerId) {
      ...
      setTimeout(() => {
        setServers(prev => {
          const srv = prev.find(s => s.id === newServerId);
          if (srv && srv.channels.length > 0) {
            const fc = srv.channels.find(c => c.type === 'text') || srv.channels[0];
  ...
  const handleSendMessage = useCallback(async (content: string, files?: File[], replyTo?: string) => {
    ...
    const insertData: any = { id: tempId, channel_id: activeChannel, user_id: user.id, author_name: profile.display_name || profile.username, content: content || '' };
  ...
  const handleRetryMessage = useCallback(async (messageId: string, content: string) => {
    ...
    const { error } = await supabase.from('messages').insert({
      id: messageId, server_id: activeServer !== 'home' ? activeServer : undefined, channel_id: activeChannel,
  ...
  const handleLeaveServer = useCallback(async () => {
    ...
    try {
      ...
      if (srv && (srv as any).leave_enabled && (srv as any).leave_message && (srv as any).leave_channel_id) {
        const displayName = profile?.username || profile?.display_name || 'Bir üye';
  ...
  const MobileBottomNav = () => {
    const isDMView = activeServer === 'home';
    const baseTabs = [
      { tab: 'servers' as MobileTab, icon: Home, label: isDMView ? 'Ana Sayfa' : i18n.t('nav.servers') },
      { tab: 'channels' as MobileTab, icon: isDMView ? MessageSquare : Hash, label: isDMView ? 'Mesajlar' : i18n.t('nav.channels') },
      { tab: 'chat' as MobileTab, icon: MessageSquare, label: i18n.t('nav.chat') },
      { tab: 'members' as MobileTab, icon: Users, label: isDMView ? 'Arkadaşlar' : i18n.t('nav.members') },
    ...
    const tabs = voice.connected
      ...
      : baseTabs;
    return (
      <div className="mobile-nav-bar w-full bg-card border-t border-border flex items-center justify-around flex-shrink-0" style={{ height: `calc(56px + env(safe-area-inset-bottom))`, paddingBottom: 'env(safe-area-inset-bottom)' }}>
            ...
            <div className="flex-1 min-h-0 overflow-hidden">
              {selectedDMUser ? (
                ...
                <DMDashboard
                  ...
                  externalPanel={mobileTab === 'members' ? 'friends' : 'dms'}
                />
              )}
            </div>
      ...
      <I18nContext.Provider value={i18n}>
        <div className="flex h-screen w-full overflow-hidden bg-background">
          ...
          <DMDashboard
            ...
            onToggleNotifications={() => setShowNotifHistory(p => !p)}
            unreadNotifCount={unreadNotifCount}
            activeDMUser={selectedDMUser}
            onCloseDM={() => { setSelectedDMUser(null); setDmAutoStartVoice(false); setDmInitiateCall(false); setDmScrollToMessageId(undefined); }}
            dmOnlineStatus={selectedDMUser ? presenceStatuses.get(selectedDMUser.userId) || 'offline' : undefined}
            dmAutoStartVoice={dmAutoStartVoice}
            dmInitiateCall={dmInitiateCall}
            dmScrollToMessageId={dmScrollToMessageId}
          />
          {!selectedDMUser && showNotifHistory && (
            <NotificationHistory
              onClose={() => setShowNotifHistory(false)}
              onUnreadCountChange={setUnreadNotifCount}
              onNavigateToMessage={handleNavigateToMessage}
    ...
    return (
      <I18nContext.Provider value={i18n}>
        <div className="relative flex flex-col w-full overflow-hidden bg-background" style={{ height: '100dvh', paddingTop: 'env(safe-area-inset-top)', paddingBottom: 0 }}>
          <div className="flex-1 min-h-0 overflow-hidden">
            ...
            {mobileTab === 'chat' && (
              <ChatArea
                ...
                wordFilterExemptRoleIds={server?.word_filter_exempt_role_ids}
          ...
          {showSearchPanel && (
            <div className="absolute inset-0 z-[60] bg-background" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
              <MessageSearchPanel
                serverId={activeServer}
                channelId={channel.id}
                channelName={channel.name}
