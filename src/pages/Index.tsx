...
const Index = () => {
  ...
  const fetchServers = useCallback(async () => {
    ...
    const [{ data: serversData }, { data: channelsData }, { data: categoriesData }] = await Promise.all([
      supabase.from('servers').select('*').in('id', serverIds).order('created_at', { ascending: true }),
      supabase.from('channels').select('*').in('server_id', serverIds).order('position', { ascending: true }),
      supabase.from('channel_categories').select('*').in('server_id', serverIds).order('position', { ascending: true }),
    ]);
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      if (data) {
        ...
        const messageIds = data.map((m) => m.id).filter((id) => !String(id).startsWith('bot-'));
        ...
        const avatarMap = new Map(profilesResult.data?.map((p) => [p.id, p.avatar_url]) || []);
        const nameMap = new Map(profilesResult.data?.map((p) => [p.id, (p as any).display_name || p.username]) || []);
        ...
        if (reactionsResult.data && reactionsResult.data.length > 0) {
    ...
    return (
      <I18nContext.Provider value={i18n}>
        <div className="relative flex flex-col w-full overflow-hidden bg-background" style={{ height: '100dvh', paddingTop: 'env(safe-area-inset-top)', paddingBottom: 0 }}>
          ...
          {showSearchPanel && (
            <div className="absolute inset-0 z-40 bg-background" style={{ paddingTop: 'env(safe-area-inset-top)' }}>
              <MessageSearchPanel
                serverId={activeServer}
                channelId={channel.id}
                channelName={channel.name}
