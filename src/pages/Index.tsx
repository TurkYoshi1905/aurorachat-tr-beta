...
const Index = () => {
  ...
  const fetchMembers = useCallback(async () => {
    ...
    const [profilesRes, memberRolesRes, serverRolesRes] = await Promise.all([
      supabase.from('profiles').select('id, username, display_name, avatar_url, status, bio, banner_color, is_premium, has_premium_badge, has_basic_badge').in('id', userIds),
  ...
  useEffect(() => {
    ...
    const mapRawMessages = async (rawData: any[]): Promise<DbMessage[]> => {
      ...
      return rawData.map((m) => {
        ...
        return {
          id: m.id,
          author: deletedAuthor || (m as any).author_name || nameMap.get(m.user_id) || 'Kullanıcı',
          avatar: (deletedAuthor ? 'D' : ((m as any).author_name || nameMap.get(m.user_id) || '?')).charAt(0).toUpperCase(),
    ...
    const fetchMessages = async () => {
      const { data } = await supabase
        .from('messages')
        .select('id, channel_id, user_id, content, inserted_at, updated_at, author_name, attachments, reply_to, is_pinned, parent_id, deleted_user_id')
  ...
  const loadMoreMessages = useCallback(async () => {
    ...
    const mapRawMessages = async (rawData: any[]): Promise<DbMessage[]> => {
      ...
      return rawData.map((m: any) => {
        ...
        return {
          id: m.id,
          author: deletedAuthor || m.author_name || nameMap.get(m.user_id) || 'Kullanıcı',
          avatar: (deletedAuthor ? 'D' : (m.author_name || nameMap.get(m.user_id) || '?')).charAt(0).toUpperCase(),
          avatarUrl: isBot ? '/aurora-bot-avatar.jpg' : (m.user_id ? (avatarMap.get(m.user_id) || null) : null),
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, async (payload) => {
        ...
        if (m.channel_id === channelRef.current) {
          ...
          setMessages((prev) => {
            if (prev.some((msg) => msg.id === m.id)) return prev;
            return [...prev, {
              id: m.id, author: rtDeletedAuthor || m.author_name || (prof as any)?.display_name || (prof as any)?.username || 'Kullanıcı',
              avatar: (rtDeletedAuthor ? 'D' : (m.author_name || '?')).charAt(0).toUpperCase(),
  ...
  useEffect(() => {
    const globalRealtime = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'channel_categories' }, () => { fetchServers(); })
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles' }, (payload) => {
        ...
        setMembers((prev) => prev.map((m) => m.id === updated.id ? { ...m, name: newName, avatar: newAvatar, avatarUrl: updated.avatar_url || null } : m));
    ...
    presenceChannel
      ...
      .subscribe(async (status) => {
        if (status === 'SUBSCRIBED') {
          ...
          // Persist platform to DB so UserProfileCard can read it
          supabase.from('profiles').update({ platform: myPlatform } as any).eq('id', user.id).then(() => {});
        }
      });
