...
const Register = () => {
  ...
  const validate = (): boolean => {
    ...
    if (step === 'email') {
      ...
      if (!captchaToken) e.captcha = t('auth.captchaRequired') || 'Lütfen robot olmadığınızı doğrulayın';
    }
    setErrors(e);
            ...
            {step === 'email' && (
              <div className="space-y-4">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  {errors.captcha && <p className="text-destructive text-xs flex items-center gap-1 self-start">⚠ {errors.captcha}</p>}
                </div>
                {/* Privacy & Terms note */}
                <p className="text-[11px] text-muted-foreground text-center leading-relaxed">
                  Kayıt olarak{' '}
                  <Link to="/privacy" target="_blank" className="text-primary hover:underline">Gizlilik Politikası</Link>'nı ve{' '}
                  <button type="button" onClick={() => setStep('terms')} className="text-primary hover:underline">Kullanım Koşulları</button>'nı kabul etmiş olursunuz.
                </p>
              </div>
            )}
