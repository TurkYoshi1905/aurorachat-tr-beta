...
const TwoFactorSection = () => {
  ...
  const handleConfirmDisable = async () => {
    ...
    try {
      ...
      if (!factor) { toast.error('2FA faktörü bulunamadı'); setDisabling(false); return; }

      // Create a challenge and verify the OTP to elevate to AAL2
      const { data: challenge, error: challengeError } = await supabase.auth.mfa.challenge({ factorId: factor.id });
      if (challengeError) { toast.error(challengeError.message); setDisabling(false); return; }
      ...
      if (unenrollError) { toast.error(unenrollError.message); setDisabling(false); return; }
      ...
      toast.error('2FA kapatılırken hata oluştu');
    } finally {
      setDisabling(false);
    }
  };
  ...
  return (
    <>
      ...
      <Dialog open={showDialog} onOpenChange={(open) => { setShowDialog(open); if (!open) { setQrUri(''); setFactorId(''); setOtp(''); } }}>
      ...
      <Dialog open={showDisableDialog} onOpenChange={(open) => { setShowDisableDialog(open); if (!open) setDisableOtp(''); }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1" onClick={() => setShowDisableDialog(false)}>İptal</Button>
              <Button variant="destructive" className="flex-1" onClick={handleConfirmDisable} disabled={disabling || disableOtp.length !== 6}>
                {disabling ? <div className="w-4 h-4 border-2 border-destructive-foreground/30 border-t-destructive-foreground rounded-full animate-spin" /> : '2FA\'yı Kapat'}
              </Button>
            </div>
          </div>
        </DialogContent>
        ...
        <div className="w-full max-w-2xl py-6 md:py-10 px-4 md:px-10 overflow-y-auto">
          {activeTab === 'account' && (
            <div className="space-y-6">
              ...
              {/* Premium Status Card */}
              {profile?.is_premium && (
                <div className="rounded-xl border border-primary/30 bg-gradient-to-r from-primary/10 to-primary/5 p-4 md:p-5 space-y-2">
                  <div className="flex items-center gap-2">
                    <Gem className="w-5 h-5 text-primary drop-shadow-[0_0_6px_hsl(var(--primary)/0.8)]" />
                    <p className="text-sm font-bold text-primary">AuroraChat Premium Aktif</p>
                  </div>
                  <p className="text-xs text-muted-foreground">Premium üyeliğiniz aktif. Tüm özel özelliklerden yararlanabilirsiniz.</p>
                  <div className="flex flex-wrap gap-2 pt-1">
                    {['Özel Rozet', 'Animasyonlu Avatar', 'Banner', '50MB Yükleme', 'Öncelikli Destek'].map(f => (
                      <span key={f} className="text-[11px] px-2 py-0.5 rounded-full bg-primary/20 text-primary border border-primary/30 font-medium">{f}</span>
                    ))}
                  </div>
                </div>
              )}
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-4 p-3 rounded-lg bg-blue-500/8 border border-blue-500/20">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">Windows 10/11 · 64-bit · ~136MB</p>
                    </div>
                  </div>
                  <a
                    href="https://github.com/TurkYoshi1905/aurorachat-tr-beta/raw/main/public/AuroraChat-Windows-x64.zip"
                    target="_blank"
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-4 p-3 rounded-lg bg-purple-500/8 border border-purple-500/20">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">Windows 7/8/10/11 · 32-bit · ~117MB</p>
                    </div>
                  </div>
                  <a
                    href="https://github.com/TurkYoshi1905/aurorachat-tr-beta/raw/main/public/AuroraChat-Windows-ia32.zip"
                    target="_blank"
                    rel="noopener noreferrer"
            ...
            <div className="space-y-5">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center justify-center gap-2">
                      <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-primary/15 border border-primary/30 text-primary text-xs font-bold">
                        <Sparkles className="w-3 h-3" /> v0.6.2
                      </span>
                      <span className="text-xs text-muted-foreground">31 Mart 2026</span>
                    </div>
                  </div>
                  <p className="text-sm text-muted-foreground leading-relaxed max-w-sm">
                    Discord benzeri gerçek zamanlı sohbet platformu. Sunucular, kanallar, doğrudan mesajlar, sesli/görüntülü odalar ve daha fazlası.
                  </p>
              ...
              <div className="rounded-xl border border-border bg-card/50 px-4 py-3 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono font-semibold text-foreground">2026.03.29</span>
                </div>
                <div className="flex items-center gap-2">
