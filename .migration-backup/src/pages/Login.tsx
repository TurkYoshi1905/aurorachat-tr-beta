...
const Login = () => {
  ...
  // Ban Modal state
  const [banModal, setBanModal] = useState<{ open: boolean; reason: string | null; bannedAt: string | null; email: string | null }>({
    open: false,
    reason: null,
    bannedAt: null,
    email: null,
  ...
  useEffect(() => {
    // v1.0.3: Eski ?banned=1 URL bazlı akış AccountBanModal'a taşındı.
    // Geriye dönük uyumluluk için URL parametresi geldiyse hâlâ uyarıyı göster.
  ...
  const checkActiveAccountBan = async (userId: string) => {
    const { data } = await (supabase.from('account_bans') as any)
      ...
      .eq('active', true)
      .maybeSingle();
    return data as { reason?: string | null; banned_at?: string | null } | null;
  };
  ...
  return (
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      ...
      <LoginBanModal
        ...
        bannedAt={banModal.bannedAt}
        email={banModal.email}
        onClose={() => setBanModal({ open: false, reason: null, bannedAt: null, email: null })}
      />
    </div>
  );
};
