...
export async function refreshSpotifyToken(refreshToken: string): Promise<{
  ...
  if (response.status === 400 || response.status === 401) throw new Error('TOKEN_REFRESH_REVOKED');
  if (response.status === 403) throw new Error('TOKEN_REFRESH_FORBIDDEN');
...
export async function getSpotifyProfile(accessToken: string) {
  ...
  if (response.status === 403) throw new Error('SPOTIFY_403');
  if (response.status === 401) throw new Error('SPOTIFY_401');
