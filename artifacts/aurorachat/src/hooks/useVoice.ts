...
// Module-level SPA navigation flag: set to true when React Router navigates within the app.
// This prevents pagehide/visibilitychange from incorrectly stopping voice/screen share.
let _isSpaNavigation = false;
export const setSpaNavigation = (v: boolean) => { _isSpaNavigation = v; };
...
export const useVoice = (): VoiceState => {
  ...
  const outputGainNodesRef = useRef<GainNode[]>([]);
  ...
  useEffect(() => {
    // Track in-app SPA navigation to avoid false-positive pagehide disconnects.
    // When user navigates within the app (pushState), we set this flag so pagehide
    // won't mistakenly disconnect the voice room.
    const markSpaNav = () => {
      _isSpaNavigation = true;
      setTimeout(() => { _isSpaNavigation = false; }, 2000);
    };
    const handlePopState = () => markSpaNav();
    window.addEventListener('popstate', handlePopState);

    // Also intercept history.pushState / replaceState (React Router uses these)
    ...
    // pagehide: only disconnect if this is NOT an in-app SPA navigation.
    // On mobile WebViews, pagehide can fire during React Router navigation which would
    // incorrectly disconnect the voice room.
    const handlePageHide = (e: PageTransitionEvent) => {
      if (_isSpaNavigation) return;
      // e.persisted === true means page goes into bfcache — don't disconnect
  ...
  // Heartbeat: update joined_at every 90s while in voice — helps detect stale WebView sessions.
  // Uses refs for mic/camera/screen so this effect is NOT re-created on every toggle.
  useEffect(() => {
      ...
      room
        ...
        .on(RoomEvent.TrackSubscribed, (track, _pub, _participant) => {
          if (track.kind === Track.Kind.Audio) {
            ...
            // Use Web Audio GainNode for amplification support (allows > 1.0 / 100%)
            try {
              ...
              gainNode.connect(ctx.destination);
              outputGainNodesRef.current.push(gainNode);
              if (ctx.state === 'suspended') ctx.resume().catch(() => {});
            } catch {
              // Fallback: native volume (max 1.0)
              audioEl.volume = Math.min(1, outputVolumeRef.current);
            }
            audioEl.play().catch(() => {});
  ...
  const setOutputVolume = useCallback((volume: number) => {
    // Allow up to 2.0 (200%) via Web Audio GainNode
    const next = Math.max(0, Math.min(2, volume));
    outputVolumeRef.current = next;
    setOutputVolumeState(next);
    // Update GainNodes (supports amplification > 1.0)
    if (outputGainNodesRef.current.length > 0) {
      outputGainNodesRef.current.forEach((gainNode) => {
        gainNode.gain.value = next;
      });
    } else {
      // Fallback: native volume (capped at 1.0)
      audioElementsRef.current.forEach((el) => {
        el.volume = Math.min(1, next);
      });
    }
  }, []);

  const setMicGain = useCallback((gain: number) => {
    const next = Math.max(0, Math.min(2, gain));
    micGainRef.current = next;
    setMicGainState(next);
    if (micGainNodeRef.current) {
      micGainNodeRef.current.gain.value = next;
    }
  }, []);
  ...
  const flipCamera = useCallback(async () => {
    ...
    try {
      ...
      // Use { ideal } so Android WebView (Sketchware) honors the back camera
      // request reliably even when enumerateDevices() returns empty deviceIds.
      await lp.setCameraEnabled(false);
      await lp.setCameraEnabled(
        true,
        { facingMode: { ideal: newMode } as any, resolution: { width: 1920, height: 1080, frameRate: 60 } } as any,
    ...
    try {
      if (!cameraEnabled) {
        await roomRef.current.localParticipant.setCameraEnabled(
          true,
          { facingMode: { ideal: 'user' } as any, resolution: { width: 1920, height: 1080, frameRate: 60 } } as any,
          AURORA_VIDEO_PUBLISH_OPTIONS as any
        );
        setFacingMode('user');
        ...
        // Always disable the camera when toggling off — never silently flip on mobile.
        // Mobile users have a separate "Switch Camera" button (flipCamera) for back/front swap.
  ...
  }, [cameraEnabled, buildParticipants, user]);
  ...
  const toggleScreenShare = useCallback(async () => {
    ...
    try {
      if (newState) {
        ...
        // Guard: don't clean up if the track ended due to SPA navigation (some
        // Android WebViews fire ended during pushState navigation).
