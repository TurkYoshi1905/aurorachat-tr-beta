...
import {
  ...
  Eye, Maximize2, Minimize2, RotateCw, ChevronUp, Settings2,
} from 'lucide-react';
...
const ScreenShareView = ({ sharerIdentity, room }: { sharerIdentity: string; room: Room | null }) => {
  ...
  // Called from a direct user tap — always succeeds on mobile
  const playFromGesture = useCallback(() => {
    ...
    video.play()
      ...
      .catch(() => {
        video.muted = true;
        video.play().then(() => markAttached()).catch(() => {});
      });
  }, [markAttached]);
  ...
  useEffect(() => {
    ...
    const attachTrack = () => {
      ...
      if (pub?.videoTrack) {
        ...
        setTrackReady(true);
        // Attempt autoplay — may silently fail on mobile; overlay handles it
        video.play().then(() => markAttached()).catch(() => {});
        return true;
      }
      return false;
    ...
    const scheduleAttach = () => {
      ...
      retryTimer = setTimeout(() => {
        if (!trackAttachedRef.current && !attachTrack()) scheduleAttach();
      }, 300);
    };

    const handleTrackSubscribed = () => {
      if (retryTimer) clearTimeout(retryTimer);
      if (!trackAttachedRef.current) attachTrack();
    };

    // Events that confirm the video is truly playing
    const onActive = () => markAttached();
    video.addEventListener('playing', onActive);
    video.addEventListener('timeupdate', onActive);

    // Resume after tab visibility change
    const handleVisibility = () => {
      if (!document.hidden && video.paused && trackAttachedRef.current) {
        video.play().then(() => markAttached()).catch(() => {});
      }
    };
    document.addEventListener('visibilitychange', handleVisibility);

    // Poll currentTime — catches browsers that don't fire events reliably
    pollTimer = setInterval(() => {
      if (!attachedRef.current && video.currentTime > 0) markAttached();
    }, 300);

    if (!attachTrack()) scheduleAttach();
    room.on(RoomEvent.TrackSubscribed, handleTrackSubscribed);

    // 20s hard timeout
    timeoutTimer = setTimeout(() => {
      if (!attachedRef.current) setLoadTimeout(true);
    }, 20000);
  ...
  }, [sharerIdentity, room, isLocal, markAttached]);
  ...
  }, [room, sharerIdentity, markAttached]);
  ...
  return (
    <div className="relative w-full h-full">
      <video
        ...
        style={{ background: '#000', objectFit: 'contain', maxWidth: '100%', maxHeight: '100%' }}
      />

      {/* Overlay — shown until video is confirmed playing.
          The ENTIRE area is a tap target so any touch on mobile triggers play(). */}
      {!attached && !loadTimeout && (
        <button
          onClick={playFromGesture}
          className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-black rounded-xl"
          style={{ WebkitTapHighlightColor: 'transparent' }}
        >
          {trackReady ? (
            <>
              <div className="w-20 h-20 rounded-full bg-[#00ff88]/15 border border-[#00ff88]/40 flex items-center justify-center animate-pulse">
                <MonitorPlay className="w-9 h-9 text-[#00ff88]" />
              </div>
              <div className="text-center">
                <p className="text-sm font-semibold text-white">Oynatmak için dokun</p>
                <p className="text-xs text-white/40 mt-1">Ekrana herhangi bir yere dokunun</p>
              </div>
            </>
          ) : (
            <>
              ...
              <div className="text-center">
                <p className="text-xs text-white/50">Ekran paylaşımı yükleniyor...</p>
                <p className="text-[11px] text-white/25 mt-1">Başlatmak için dokunabilirsiniz</p>
              </div>
            </>
          )}
        </button>
      ...
      {/* Hard timeout — show retry */}
      {loadTimeout && !attached && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-black rounded-xl">
          <MonitorPlay className="w-8 h-8 text-yellow-400/70" />
          <span className="text-xs text-yellow-300/70 text-center px-4">
            Ekran paylaşımı bağlanamadı.<br />Paylaşan kişi bağlantısını kontrol etsin.
          </span>
          <button
            onClick={handleRetry}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-white/10 hover:bg-white/20 text-white/70 transition-colors"
          >
            <RotateCw className="w-3.5 h-3.5" /> Yeniden Dene
          </button>
        </div>
      )}
    </div>
...
const VoiceMeetingRoom = ({ voiceState, onToggleChat, showChat, isMobile = false, serverId }: VoiceMeetingRoomProps) => {
  ...
  const {
    ...
    voiceChannelName, voiceChannelId, outputVolume, micGain, toggleMic, toggleDeafen, toggleCamera, flipCamera, toggleScreenShare, toggleSystemAudio, setOutputVolume, setMicGain, disconnect,
  } = voiceState;
    ...
    <div className="flex-1 flex flex-col min-w-0 overflow-hidden h-full" style={{ background: '#0f0f0f' }}>
      ...
      >
        <div className={`flex items-center justify-center ${isMobile ? 'gap-1.5' : 'gap-2'} flex-wrap`}>
          {/* Mic + Device Settings chevron */}
          <div className="relative flex flex-col items-center">
            {/* Device settings popover */}
            {showDeviceSettings && (
              <div
                className="absolute bottom-full mb-2 left-1/2 -translate-x-1/2 z-50 w-56 rounded-2xl border border-white/10 shadow-2xl p-3 space-y-3"
                style={{ background: 'rgba(18,18,22,0.97)', backdropFilter: 'blur(20px)' }}
              >
                <div className="flex items-center gap-1.5 mb-1">
                  <Settings2 className="w-3.5 h-3.5 text-white/40" />
                  <span className="text-[11px] font-semibold text-white/50 uppercase tracking-wider">Ses Ayarları</span>
                </div>
                {/* Speaker Volume */}
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-white/40 font-medium">Hoparlör</span>
                    <span className="text-[10px] text-white/50 w-8 text-right">{Math.round(outputVolume * 100)}%</span>
                  </div>
                  <input
                    type="range"
                    min={0}
                    ...
                    className="w-full accent-[#00ff88] h-1.5"
                    aria-label="Hoparlör seviyesi"
                  />
                  {outputVolume > 1 && (
                    <p className="text-[9px] text-yellow-400/70">Amplifikasyon aktif</p>
                  )}
                </div>
                {/* Mic Gain */}
                <div className="space-y-1 border-t border-white/8 pt-2">
                  <div className="flex items-center justify-between">
                    <Mic className="w-3.5 h-3.5 text-[#00ff88]/60" />
                    <span className="text-[10px] text-white/40 font-medium">Mikrofon Kazancı</span>
                    <span className="text-[10px] text-white/50 w-8 text-right">{Math.round(micGain * 100)}%</span>
                  </div>
                  <input
                    type="range"
                    min={0}
            ...
            <div className="flex items-end gap-0.5">
              ...
              {/* Cihaz ayarları açma/kapama oku */}
              <button
                onClick={() => setShowDeviceSettings(v => !v)}
                title="Ses cihazı ayarları"
                className={`mb-1 w-6 h-6 rounded-md flex items-center justify-center transition-all ${
                  showDeviceSettings
                    ? 'bg-[#00ff88]/30 text-[#00ff88] ring-1 ring-[#00ff88]/50'
                    : 'bg-white/15 text-white/70 hover:text-white hover:bg-white/25'
                }`}
              >
                <ChevronUp className={`w-3.5 h-3.5 transition-transform ${showDeviceSettings ? 'rotate-180' : ''}`} />
              </button>
            </div>
          </div>
          ...
          {/* Compact speaker level indicator (click to open device settings) */}
          {!isMobile && (
            <button
              onClick={() => setShowDeviceSettings(v => !v)}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl transition-all ring-1 ${
                showDeviceSettings
                  ? 'bg-[#00ff88]/10 ring-[#00ff88]/30 text-[#00ff88]'
                  : 'bg-white/5 ring-white/10 text-white/40 hover:text-white/70 hover:bg-white/8'
              }`}
              title="Ses cihazı ayarları"
            >
              {outputVolume <= 0 ? <VolumeX className="w-3.5 h-3.5 shrink-0" /> : <Volume2 className="w-3.5 h-3.5 shrink-0" />}
              <span className="text-[10px] font-medium">{Math.round(outputVolume * 100)}%</span>
